...
class Manager
{
    ...
    /**
     * Retrieve a token for someone installing your embedded Shopify App.
     *
     ...
     * @return Variant
     */
    public function getCustomCollection($id = null)
    {
        return $this->fetchFromApiCache(Collection::class, $id)
            ?: new CustomCollection($this->client, $id);
    }
     ...
     */
    public function getAllCustomCollections(array $params = [])
    {
        $collections = $this->fetchFromApiCache(Collection::class, $params)
            ?: (new CustomCollection($this->client))->all($params);

        return defined('LARAVEL_START') ? collect($collections) : $collections;
    }
